...
namespace Drupal\ictv_sequence_classifier\Plugin\Block;

use Drupal\Core\Block\BlockBase;


/**
 * A Block for the ICTV sequence classifier form.
 *
 * @Block(
 *   id = "ictv_sequence_classifier_block",
 *   admin_label = @Translation("ICTV Sequence Classifier block"),
 *   category = @Translation("ICTV"),
 * )
 */
class IctvSequenceClassifierBlock extends BlockBase {
    ...
    public function build() {
        ...
        if (!$currentUser) { 
            \Drupal::logger('ictv_sequence_classifier')->error("Current user is invalid"); 
        ...
        $build = [
            '#markup' => $this->t("<div id=\"ictv_sequence_classifier_container\" class=\"ictv-custom\"></div>"),
            '#attached' => [
                'library' => [
                    'ictv_sequence_classifier/ICTV-SequenceClassifier',
                ],
                'library' => [
                    'ictv_sequence_classifier/sequenceClassifier',
                ],
            ],
        ];
    ...
    public function loadData() {
        ...
        if (!$query) { \Drupal::logger('ictv_sequence_classifier')->error("Invalid query object"); }

        $result = $query->fetchAll();
        if (!$result) { \Drupal::logger('ictv_sequence_classifier')->error("Invalid result object"); }
