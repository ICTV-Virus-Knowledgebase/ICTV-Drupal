...
BEGIN
   ...
   UNION ALL (
      ...
      SELECT 
         ...
         CASE WHEN tn.abbrev_csv IS NULL THEN '' ELSE '"'+tn.abbrev_csv+'"' END+','+
         CASE WHEN tn.cleaned_name IS NULL THEN '' ELSE '"'+tn.cleaned_name+'"' END+','+
         CASE WHEN tn.exemplar_name IS NULL THEN '' ELSE '"'+tn.exemplar_name+'"' END+','+
         CASE WHEN tn.genbank_accession_csv IS NULL THEN '' ELSE '"'+tn.genbank_accession_csv+'"' END+','+
         CASE WHEN tn.isolate_csv IS NULL THEN '' ELSE '"'+tn.isolate_csv+'"' END+','+
	      CAST(tn.msl_release_num AS VARCHAR(12))+','+
         CASE WHEN tn.[name] IS NULL THEN '' ELSE '"'+tn.[name]+'"' END+','+
         CASE WHEN tl.[name] IS NULL THEN '' ELSE '"'+tl.[name]+'"' END+','+
         CASE WHEN tn.refseq_accession_csv IS NULL THEN '' ELSE '"'+tn.refseq_accession_csv+'"' END+','
