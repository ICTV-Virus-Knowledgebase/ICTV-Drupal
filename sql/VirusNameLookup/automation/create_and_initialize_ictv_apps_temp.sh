...
mariadb -D $AppsTempDB -s -b --show-warnings < CreateVocabularyTable.sql
mariadb -D $AppsTempDB -s -b --show-warnings < CreateTermTable.sql

# Initialize the vocabulary and term tables (after adding the stored procedure).
echo -e "\nInitializing vocabulary and term tables"
mariadb -D $AppsTempDB -s -b --show-warnings < InitializeVocabularyAndTerms.sql
echo "CALL InitializeVocabularyAndTerms();" | mariadb -D $AppsTempDB -s -b --show-warnings

# Create the NCBI Taxonomy tables.
echo -e "\nCreating NCBI Taxonomy tables"
mariadb -D $AppsTempDB -s -b --show-warnings < CreateNcbiTaxonomyTables.sql
...
mariadb -s -b --show-warnings < PopulateTempNcbiTaxonomyTables.sql
...
mariadb -D $AppsTempDB -s -b --show-warnings < CreateLatestReleaseOfIctvIDTable.sql

# Create the searchable_taxon table.
echo -e "\nCreating searchable_taxon table"
mariadb -D $AppsTempDB -s -b --show-warnings < CreateSearchableTaxonTable.sql
