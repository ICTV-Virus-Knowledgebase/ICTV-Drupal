...
class ExportToCSV:
   ...
   #-----------------------------------------------------------------------------------
   def exportTaxonomyNode(self, dbServer_, ictvDB_):
      ...
      # Create the command line text to run sqlcmd.
      cmd = (f"sqlcmd -S {dbServer_} "
            f"-Q \"EXEC [{ictvDB_}].dbo.exportTaxonomyNodesAsCSV @mslRelease = NULL \" "
            f"-o \"{filename}\" "
            "-y 0 ")
...
if __name__ == '__main__':
   ...
   dbServer = args.dbServer[0]
   ictvDB = args.ictvDB[0]
   table = args.table[0]
   ...
   match table:
      ...
      case "taxonomy_node":
         exportToCSV.exportTaxonomyNode(dbServer, ictvDB)
