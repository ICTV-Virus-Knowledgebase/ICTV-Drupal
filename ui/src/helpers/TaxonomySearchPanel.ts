...
export class TaxonomySearchPanel {
    ...
    selectionHandler: ISelectionHandler = null;
    ...
    // C-tor
    constructor(containerSelector_: string, searchContext_: SearchContext, selectionHandler_: ISelectionHandler) {
    ...
    async initialize() {
        ...
        this.elements.searchResults.addEventListener("click", async (event_: MouseEvent) => {
            ...
            if (buttonEl.classList.contains("view-search-result-button")) {
                ...
                if (this.searchContext === SearchContext.VisualTaxonomy) {
                    lineage = buttonEl.getAttribute("data-lineage");
                    if (!lineage) { throw new Error("Unable to select search result: Invalid lineage"); }
                }
