...
class TaxaBlastJob {
   ...
   /*
   // Create an invalid TaxaBLAST Job "object" (nested arrays) to return when a job is not found or is invalid.
   ...
   // Return a TaxaBLAST Job "object" (nested arrays) from a row of database results.
   public static function fromArray(array $row) {
      ...
      if (!Utils::isNullOrEmpty($json)) { 
         ...
         if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
            \Drupal::logger(Common::$MODULE_NAME)->error("JSON data is invalid after conversion in fromArray");
         }
      }
   ...
   // Get the job and result metadata generated by TaxaBLAST.
   public static function getJob(Connection $connection, string $jobUID) {
      ...
      // Generate SQL to call the "getTaxaBlastJob" stored procedure.
      $sql = "CALL getTaxaBlastJob(:jobType, :jobUID);";
      ...
      return TaxaBlastJob::fromArray($row);
   }
   ...
   public static function searchJobs(Connection $connection, ?string $searchText, string $userUID) {
      ...
      foreach($rows as $row) {
         ...
         $job = TaxaBlastJob::fromArray($row);
         array_push($jobs, $job);
      }
